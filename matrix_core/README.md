# matrix core
this folder contains a simple example to showcase the usage of AMD's matrix core. we use `__builtin_hcu_mmac_f32_16x16x16_f16` instruction with single wave to show case the layout requirement of this instruction. this instruction is supported for NMZ, BMZ serious of AMDGPU (change arch number inside rebuild.sh to build on proper arch)
