                   ...
                   int stride_c)
{
    ...
    fp16x8_t v_a;
    v_a.lo = *reinterpret_cast<const fp16x4_t*>(reinterpret_cast<const fp16_t*>(ptr_a) + offset_a);
    v_a.hi = *reinterpret_cast<const fp16x4_t*>(reinterpret_cast<const fp16_t*>(ptr_a) + offset_a + 8);
...
#if 0
    int offset_c = (((threadIdx.x >> 4) << 3) * stride_c) + (threadIdx.x & 0xf);

    #pragma nuroll
    for(auto i = 0; i < 8; i++) {
        // if(threadIdx.x==0)
        // {
        //     printf("i:%d, row_offset:%d\n", i, row_offset);
        // }
    ...
    for (int i = 0; i < 8; ++i) {
        int r = row_start + i;
        lds_wave[r * 16 + col] = __float2half(v_c[i]);
    }
    __syncthreads();
    ...
    int4_t val_128 = *reinterpret_cast<const int4_t*>(&lds_wave[r_write * 16 + c_write]);

    __half* out_ptr = reinterpret_cast<__half*>(ptr_c) + r_write * stride_c + c_write;
    
    *reinterpret_cast<int4_t*>(out_ptr) = val_128;
...
int main(int argc, char ** argv)
{
    ...
    {
        matrix_core_kernel_standard<<<1, 32>>>(dev_a, dev_b, dev_c, lda, ldb, ldc);

        HIP_CALL(hipMemcpy(fp16_c, dev_c, ldc*m*sizeof(float16), hipMemcpyDeviceToHost));
        bool res = valid_vector( host_c, fp16_c, m*n, 1e-3);
        printf("[16x16x16_gfx12, standard], %s",res?"valid":"fail");fflush(stdout);
        printf("\n"); fflush(stdout);
    }
